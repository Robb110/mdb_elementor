...
class FullscreenHamburgerMenu extends Widget_Base
{
    ...
    protected function render(){
        ...
        <div class="main-menu">
			...
			<ul class="nav flex-column delay-1s">
	...
	foreach ((array) $menuItems as $key => $menu_item) {
		...
		if ($menu_item->menu_item_parent == 0) {
		   	...
		   	if ($menuItems[ $count + 1 ]->menu_item_parent == $parent_id )
			{
				...
				$addClassItem .= $settings['submenu_display_mode'];
				$itemAction = "";
				?>
		...
		// if this item has a (nonzero) parent ID, it's a second-level (child) item
		   ...
		   ?>
		   <li class="nav-item" onclick="closeFullscreenHamburgerMenu('<?php echo 'hamburger-menu-' . $this->get_id() . '-body'; ?>')"><a class="nav-link <?php echo $settings['elements_hover_effect']; ?>" href="<?php echo $menu_item->url; ?>"><?php echo $menu_item->title; ?></a></li>
		   <?php
...
?>
		...
		<script>
			jQuery("<?php echo '.elementor-element-' . $this->get_id(); ?> .has-submenu.fhm-dropdown").on('click', function(){
				let submenu = jQuery(this).children('.submenu');
				if($(submenu).hasClass('active')){
					$(submenu).removeClass('active');
					jQuery(this).children('.nav-link').removeClass('arrow');
				}else{
					$(submenu).addClass('active');
					jQuery(this).children('.nav-link').addClass('arrow');
				}
			});
			jQuery("<?php echo '.elementor-element-' . $this->get_id(); ?> .has-submenu.fhm-page").on('click', function(e){
				if(!$(e.target).hasClass('.submenu') && $(e.target).parents(".submenu").length <= 0){
					let submenu = jQuery(this).children('.submenu').first();
